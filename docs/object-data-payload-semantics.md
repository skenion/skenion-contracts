...
| Port payload acceptance metadata | `PortSpecV01.accepts`, object-spec `instancePorts[].accepts`, graph edge validation | Keep as payload compatibility metadata. It must not be copied into `node.kind`. |
